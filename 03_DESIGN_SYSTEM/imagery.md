...
**As built (16 September 2026):** the first delivery is generated imagery. Rule 1 is applied in the build —
every frame that shows the venue, a pavilion, a stage, the room or ATHLIMA signage is declared
`impression: true` in `web/src/content/media.ts` and `MediaSlot` renders "Artist's impression" on it;
frames of the city, a building or an athlete alone are not labelled (rule 3). Rule 2 holds: no generated
person is named or presented as a participant; every people frame is decorative (empty alt) and the one
meaningful image (`home.athlima20`) describes athletes, not participants. The slots-by-slot state, the
two frames used under a crop because they carried baked type, and what re-delivery must fix are in
`08_OPERATIONS/asset-brief.md` §3a.
...
| Optimisation | AVIF primary, WebP fallback. A repository still: `web/scripts/build-media.ts` at build, served as a native `<picture>` with a blur placeholder, immutable-cached, the hero preloaded. A CMS still: `next/image` from the Sanity CDN. Never upscaled — a frame below spec is served at its own size and recorded as `belowSpec` in the manifest. |
